...
{
public:
  static vtkTransformBoxWidgetCallback *New()
    { return new vtkTransformBoxWidgetCallback; }
  virtual void Execute(vtkObject *caller, unsigned long, void*)
    {
      vtkBoxWidget *widget = reinterpret_cast<vtkBoxWidget*>(caller);
      if (this->Mapper)
      {
        ...
        this->Mapper->SetSecondInputUserTransform(t);
        t->Delete();
      }
    }
...
void PrintUsage()
{
  cout << "Usage: " << endl;
  cout << endl;
  cout << "  GPURenderDemo  <voloptions> -vol2 <voloptions> <options>" << endl;
  cout << endl;
...
void ParseArguments(int argc, char *argv[])
{
  ...
  while ( count < argc )
    ...
    else if ( !strcmp( argv[count], "-vol2" ) )
      {
        current_volume = 1;
        count++;
      }
...
//------------------------------------------------------------------------------
void CreateTransferFunctions( const VolumeInfo& vi, 
...
int main(int argc, char *argv[])
{
  ...
  vtkSmartPointer<vtkAlgorithm>     reader  [2];
  vtkSmartPointer<vtkImageData>     input   [2];
  vtkSmartPointer<vtkImageResample> resample[2];

  // transfer functions and properties 
  vtkSmartPointer<vtkColorTransferFunction>  colorFun   [2];
  vtkSmartPointer<vtkPiecewiseFunction>      opacityFun [2];
  vtkSmartPointer<vtkVolumeProperty>         property   [2];
...
/*    // Set the sample distance on the ray to be 1/2 the average spacing
    ...
    // options
    CreateTransferFunctions(vol_info[vol], colorFun[vol], opacityFun[vol], 
                            property[vol], mapper);
    ...
    if (vol==0)
      ...
      mapper->SetProperty2( property[vol] );
  ...
  // Add a transform box widget if the clip option was selected
  vtkSmartPointer<vtkBoxWidget> transform_box = vtkBoxWidget::New();
  transform_box->SetInteractor(iren);
  ...
  transform_box->InsideOutOn();
  transform_box->PlaceWidget();
  vtkSmartPointer<vtkTransformBoxWidgetCallback> transform_callback = 
    vtkTransformBoxWidgetCallback::New();
  transform_callback->SetMapper(mapper);
  transform_box->AddObserver(vtkCommand::InteractionEvent, transform_callback);
  transform_box->EnabledOn();
  transform_box->GetSelectedFaceProperty()->SetOpacity(0.0);
